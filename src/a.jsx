...
function App() {
  ...
  const [userId] = useState("26a783e4-ed70-4507-946b-d8918cb39cc3") // Replace with actual user ID from auth
  ...
  // Fetch chat history for a selected chat room
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setChatHistory(prev => {
        ...
        updatedHistory[updatedHistory.length - 1].response = responseData.responce
  ...
  // Poll for response until available
    ...
    for (let i = 0; i < maxAttempts; i++) {
      try {
        ...
        if (response.status === 200 && response.data.responce) {
          return response.data // Return the response data with 'responce'
        }
      } catch (error) {
        console.error("Polling error:", error)
        if (error.response && error.response.status !== 200) {
          console.log(`Non-200 status: ${error.response.status}, continuing to poll...`)
        }
      }
                ...
                <div key={index} className="mb-4 max-w-[80%]">
                  ...
                  {chat.response && (
                    <div className="bg-gray-600 mt-2 mr-auto p-3 rounded-lg">
                      <Text className="!text-white">Assistant: {chat.response}</Text>
                    </div>
                  )}
                </div>
              ))}
