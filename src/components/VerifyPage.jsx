...
function VerifyPage() {
  ...
  const handleResendCode = async () => {
    try {
      setTimer(60);
      setCode(["", "", "", "", "", ""]);
      await axios.post("https://backend.amur1.uz/users/resend-code", {
  ...
  const handleVerify = async () => {
    ...
    try {
      const response = await axios.post("https://backend.amur1.uz/users/verify-code", {
  ...
  const handleModalOk = async () => {
    ...
    try {
      await axios.post(`https://backend.amur1.uz/users/update?id=${localStorage.getItem("user_id")}`, {
        full_name: fullName,
        phone_number: phoneNumber,
      });
